...
class VS_host(threading.Thread):
    def __init__(self, port=8000, cert='/Users/ricardocarretero/dev/vm/ubuntu1404/certs/./test1cert.pem', key='/Users/ricardocarretero/dev/vm/ubuntu1404/certs/./test1key.pem', handler = MyRequestHandler, name= ''):
        threading.Thread.__init__(self)
        self.port = port
        self.cert = cert
        self.key = key
    ...
    def run(self):
        try:
            print 'HTTPS '+ name  + ' on port: ', self.port
            VS = BaseHTTPServer.HTTPServer(('', int(self.port)), handler)
            VS.socket = ssl.wrap_socket(VS.socket, certfile= cert, server_side=True, keyfile= key)
            VS.serve_forever()
        except KeyboardInterrupt:
            VS.close()
...
class VS2Handler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def send_head(self):
        ...
        fs = os.fstat(f.fileno())
        self.send_header('Content-Length', str(fs[6])) #Used for TCP connections
        self.send_header('Last-Modified', self.date_time_string(fs.st_mtime))
        self.end_headers()
        return f
...
class IOitems(object):
    ...
    def startServers(self):
        ...
        handlers = [ VS1Handler, VS2Handler, VS3Handler]
        ...
        for number in range(3) 
          VS_servers[number] = VS_host(ports[number], certs[number], keys[number], handlers[number], name= str(number))
          VS_server[number].daemon = True
          VS_server[number].start()
